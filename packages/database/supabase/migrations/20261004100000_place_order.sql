-- Placing an order in one step, priced by the database.
--
-- Checkout used to save the order, then each item, then each item's options
-- as separate requests from the phone -- 30+ for a big catering order -- so
-- a dropped connection part-way left a half-saved order the kitchen could
-- see. It also trusted the prices the app sent. place_order() does all of it
-- in one transaction (everything is saved, or nothing is) and works out
-- every price itself from the menu:
--   * each item's price and its options' prices, from menu_items /
--     modifier_options at that store (unavailable items, options that don't
--     belong to the item, missing required choices and drops outside their
--     window are refused);
--   * reward lines (wheel / PaninoPoints prizes) are free only with an
--     unused code that belongs to this customer and fits that item;
--   * the applied promo, with the same rules as the app
--     (lib/promoEligibility.ts evaluatePromo);
--   * the store's tax rate;
--   * the catering rules (ordered by 6 PM the day before, $100 minimum
--     after discounts).
-- If the total differs from what the customer was shown (a price changed
-- since it went in the cart), it stops with PRICE_CHANGED and the new total
-- so the app can ask before placing it.
--
-- Customers can no longer write orders directly -- only through this.
...
-- Orders are only written through place_order() now (staff keep their own
-- "Staff manage ..." policies).
DROP POLICY IF EXISTS "Users insert own orders" ON public.orders;
DROP POLICY IF EXISTS "Users insert own order items" ON public.order_items;
DROP POLICY IF EXISTS "Users insert own item modifiers" ON public.order_item_modifiers;
