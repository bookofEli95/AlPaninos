    ...
    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)' }}>
      ...
      <View className="bg-[#FAF6F0] rounded-2xl mx-4 p-5" style={{ marginTop: 90, maxHeight: '75%' }}>
        <View className="flex-row justify-between items-center mb-1">
          <Text className="text-xl font-extrabold text-[#1C1917] flex-1 mr-2">{title}</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text className="flex-1 ml-3 font-semibold text-[#1C1917]" numberOfLines={1}>{item.name}</Text>
              <Text className="text-[#A61C14] font-extrabold text-sm">FREE</Text>
            </TouchableOpacity>
          )}
        />
      </View>
