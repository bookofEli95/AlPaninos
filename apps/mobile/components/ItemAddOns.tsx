...
export default function ItemAddOns({
  ...
  return (
    <View className="mt-6 border-t border-stone-200 pt-4">
      <Text className="text-lg font-bold text-[#1C1917] mb-2">Add to your order</Text>
        ...
        return (
          <View key={suggestion.menuItemId} className="flex-row items-center py-3 border-b border-stone-100">
            ...
            <View className="flex-1 ml-3">
              <Text className="text-[#1C1917] font-semibold">{suggestion.name}</Text>
              <Text className="text-[#A61C14] font-bold text-sm mt-0.5">${suggestion.basePrice.toFixed(2)}</Text>
            </View>
            {qty === 0 ? (
              <TouchableOpacity
                onPress={() =>
              ...
              <View className="flex-row items-center bg-stone-100 rounded-lg px-1 py-1 border border-stone-200">
                ...
                <Text className="font-bold text-[#1C1917] text-sm w-6 text-center">{qty}</Text>
                <TouchableOpacity
                  onPress={() =>
                    incrementSimpleItem(
                      { menuItemId: suggestion.menuItemId, name: suggestion.name, basePrice: suggestion.basePrice },
